...
class FromUserStorageDataToUserMapper(
...
) : Mapper<UserStorageData, User> {
    override suspend fun map(from: UserStorageData): User {
        ...
        val userLocation =
            if (from.locationId.isEmpty() || from.city.isEmpty() || from.country.isEmpty()) {
                null
            } else {
                Location(
                    id = from.locationId,
