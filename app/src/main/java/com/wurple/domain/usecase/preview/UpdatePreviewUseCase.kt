...
class UpdatePreviewUseCase(
...
) : CoroutineUseCase<Unit, UpdatePreviewUseCase.Params>(
    coroutineContext,
    errorHandler
) {
    override suspend fun executeOnBackground(params: Params) {
        ...
        previewGateway.updatePreview(currentUserId, params.id, params.createPreviewRequest)
    }

    class Params(val id: String, val createPreviewRequest: CreatePreviewRequest)
}
